...
CREATE TRIGGER prevent_users_mutation
    BEFORE UPDATE OF id, birthdate, created_at, created_by ON users
BEGIN
...
CREATE TRIGGER prevent_admin_grant_update
    BEFORE UPDATE OF id, user_id, granted_at, granted_by ON admins
BEGIN
...
CREATE TRIGGER enforce_admin_revocation_rules
    BEFORE UPDATE OF revoked_at, revoked_by ON admins
BEGIN
...
CREATE TRIGGER prevent_admin_token_identity_update
    BEFORE UPDATE OF token, user_id, created_at ON admin_tokens
BEGIN
...
CREATE TRIGGER prevent_admin_token_expiry_extension
    BEFORE UPDATE OF expires_at ON admin_tokens
